...
- 支持 Economy、Balanced、Speed 三种价格与延迟权衡策略。
- 支持单次路由、只读模拟、定时自动路由和 JSON 输出。
- 以稳定的倍率为主权重、波动较大的首字速度为次权重，避免频繁切换。
- 支持带轮转的 JSONL 审计日志，记录每个候选的评分依据。
...
顶栏可选择“跟随系统”“浅色”或“深色”，选择会保存到普通设置中。状态提示、表面和边框会随主题切换。“模拟”只计算决策，不发送更新 Key 分组的请求。界面不会打开浏览器。
...
aihub-router watch --interval 60 --json \
  --log-file ./router.jsonl --log-max-mb 20 --log-files 7
```
...
后台运行不会启动桌面或浏览器。systemd 的控制台日志可通过
`journalctl -u aihub-router.service` 查看；完整决策审计写入
`/var/log/aihub-router/router.jsonl`，默认单文件 20 MiB、保留 7 个轮转文件。
...
对于有首 Token 实测值的候选，程序以最低倍率候选为基准计算：

```text
价格溢价 = (候选倍率 - 最低倍率) / 最低倍率
速度收益 = 基准首字延迟 / 候选首字延迟 - 1
加权得分 = 首字速度权重 × 速度收益 - 倍率权重 × 价格溢价
```

最低倍率候选得分为 0。只有得分大于 0 的候选才值得多付费，最终选择得分最高者：

| 模式 | 倍率权重 | 首字速度权重 | 决策倾向 |
|---|---:|---:|---|
| Economy | 98% | 2% | 几乎只为极端速度差支付溢价 |
| Balanced | 90% | 10% | 价格优先，在速度差距很大时切换 |
| Speed | 75% | 25% | 更愿意用倍率换取首字速度 |

Balanced 中，67% 的倍率溢价要求候选至少约 7 倍快才能进入推荐，得分滞后后约需 7.5 倍快才会切换。因此
`0.03x / 6051ms` 会优于 `0.05x / 1891ms`，而当低倍率分组恶化到
`0.03x / 111032ms` 时，`0.05x / 6025ms` 才会胜出。

倍率是相对稳定的数据，因此占主要权重；首字速度容易受瞬时网络和负载影响，因此只占次要权重。当前分组仍有效时，新候选的加权得分必须比当前分组高出 `0.05` 才会切换；得分差不足时优先保持既有分组，避免因瞬时小幅波动来回切换。Balanced 使用 `90% / 10%` 的价格与速度权重，不设置固定或软延迟边界。当前分组失效时仍立即选择新的有效候选。

缺失、零值或非法首字延迟在存在有效实测候选时不参与速度竞争；倍率为零时只在免费候选中选择。每次决策都会输出原因、目标分组、价格溢价、延迟改善比例和逐 Key 更新结果。审计日志还包含所有候选的 `speedupRatio`、`weightedScore` 和推荐标记，不记录密码、Token、Cookie 或主密钥。
