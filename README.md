...
2. ❤️ 超敏感体质：最后样本时间、样本量和首字延迟稳定性共同形成 `0..1` 置信度。低置信度的低延迟会被保守修正，最后成功样本超过 15 分钟的分组不会冒充当前可用。
...
每次决策先过滤掉不可用候选：没有有效实际样本、最后样本过期、置信度低于 `0.20`、平台不匹配、账号无权限、被加入黑名单或倍率异常的分组都不会参与竞争。置信度综合样本新鲜度、有效样本量和首字延迟的稳定性。
