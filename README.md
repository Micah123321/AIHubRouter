...
| 模式 | 倍率权重 | 首字速度权重 | 决策倾向 |
|---|---:|---:|---|
| Economy | 95% | 5% | 几乎只为极端速度差支付溢价 |
| Balanced | 80% | 20% | 稳定倍率主导，速度差距很大时才切换 |
| Speed | 35% | 65% | 更愿意用倍率换取首字速度 |

Balanced 中，67% 的倍率溢价要求候选至少约 3.67 倍快才能胜出。因此
`0.03x / 6051ms` 会优于 `0.05x / 1891ms`，而当低倍率分组恶化到
`0.03x / 111032ms` 时，`0.05x / 6025ms` 才会胜出。

倍率是相对稳定的数据，因此占主要权重；首字速度容易受瞬时网络和负载影响，因此只占次要权重。稳定性来自评分本身，不使用切换冷却、最短驻留时间或连续确认次数。加权推荐一旦改变就立即执行，当前分组失效时也立即选择新的有效候选。
